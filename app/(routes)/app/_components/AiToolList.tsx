...
const AiToolList = () => {
  ...
  const tools = [
    ...
    },
    {
      ...
      type: "Photo Tool",
      image: "https://images.unsplash.com/photo-1600132806370-bf17e65e942f?q=80&w=500&auto=format&fit=crop",
    ...
    <div className="w-full max-w-6xl mx-auto px-4 py-12">
      ...
      <div className="mb-8 text-left">
        ...
        <p className="text-xs text-neutral-500 dark:text-neutral-400 mt-1">
          Select an AI powerhouse to begin generating world-class product assets.
        </p>
      </div>

      {/* 
        Grid Setup: Laptop par ek line me teen cards chalenge, 
        lekin har card ke andar text left hoga aur photo right par!
      */}
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
        {tools.map((tool, index) => (
          <div 
            key={tool.id}
            className="flex flex-row bg-white dark:bg-neutral-900 border border-neutral-200 dark:border-neutral-800 rounded-xl overflow-hidden shadow-sm transition-all duration-300 hover:shadow-md hover:border-neutral-300 dark:hover:border-neutral-700 h-48"
          >
            {/* LEFT SIDE: Text Content & Button Area */}
            <div className="flex flex-col flex-1 p-4 text-left justify-between h-full min-w-0">
              <div>
                {/* Tool Type Tag */}
                <span className={`inline-flex items-center px-2 py-0.5 rounded-full text-[9px] font-bold tracking-wide mb-1.5 ${tool.badgeColor}`}>
                  {tool.type}
                </span>

                {/* Title */}
                <h3 className="text-sm font-bold text-neutral-900 dark:text-neutral-50 tracking-tight line-clamp-1">
                  {tool.title}
                </h3>
              ...
              <div className="mt-2">
                <button 
                  type="button"
                  className="inline-flex items-center justify-center font-semibold text-[11px] rounded-lg px-3 py-1.5 bg-blue-600 hover:bg-blue-700 text-white shadow-sm transition-colors active:scale-[0.98]"
                >
                  Create Now
                </button>
              </div>
